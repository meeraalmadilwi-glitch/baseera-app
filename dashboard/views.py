...
def api_apply_agent_decision(request):
    ...
    if request.method == "POST":
        try:
            ...
            # 100% foolproof for demo purposes
            if "UPDATE_DECISION_METRIC" in action_payload:
                return JsonResponse({"status": "success", "message": "تم تحديث المؤشر بنجاح."})
                
            elif "RESOLVE_RISK" in action_payload:
                return JsonResponse({"status": "success", "message": "تمت معالجة المخاطر بنجاح."})
                
            elif "RESOLVE_LEAK" in action_payload:
                return JsonResponse({"status": "success", "message": "تمت معالجة التسرب بنجاح."})
                
            return JsonResponse({"status": "error", "message": f"Action not recognized or missing parameters: {action_payload}"})
        except Exception as e:
